...
class Plasma(object):
    ...
    def save_to_array(self):
        ...
        self.potentialstored[self.timestamp] = self.potential  
        #self.electric[self.timestamp] = self.E     
    
    
    def compute_energy(self):
        j = (self.pos/self.dx).astype(int)
        factor = ((self.pos - self.X[[j[:,0]],[j[:,1]]])/self.dx)[0]
        multiply = np.zeros((self.N,2))
        multiply[:,0]  = factor[:,0]
        multiply[:,1]  = factor[:,1]
        
        # f ( x , y ) ≈ f ( 0 , 0 ) ( 1 − x ) ( 1 − y ) + f ( 1 , 0 ) x ( 1 − y ) + f ( 0 , 1 ) ( 1 − x ) y + f ( 1 , 1 ) x y 
        #This is bilinear interpolation
        phi_part =  (self.phi + self.V)[[j[:,0]],[(j[:,1]+1)%self.n]][0]*(1-multiply[:,0])*(multiply[:,1]) + \
                    (self.phi + self.V)[[(j[:,0]+1)%self.n],[j[:,1]]][0]*(multiply[:,0])*(1-multiply[:,1]) + \
                    (self.phi + self.V)[[(j[:,0]+1)%self.n],[(j[:,1]+1)%self.n]][0]*(multiply[:,0])*(multiply[:,1])
        self.energy = np.sum(0.5*self.m*self.speed**2)
        self.potential = np.sum(0.5 * self.q*phi_part)
    ...
    def __init__(self,q,m,dx,dt,n,N,T,eps0,pos_init,speed_init,V=zero):
        ...
        self.potentialstored = np.zeros(T)
    ...
    def compute_ElectricField(self):
        ...
        x = np.fft.fftfreq(self.n) * 2 * np.pi
        xx,yy = np.meshgrid(x,x)
        K = np.zeros((self.n,self.n,2))
        K[:,:,1] = xx
        K[:,:,0] = yy
        K[0,0] = 1./np.sqrt(2)*np.array([1.,1.])
        self.phi_ = self.rho_/self.eps0/(np.linalg.norm(K,axis=2)**2)
        ...
        self.E[:,:,0] = B
        self.E[:,:,1] = C
        affichage(self.phi,self.X,self.n)
    ...
    def save_to_text(self):
        ...
        np.save("../results/2Dpotential.npy", self.potentialstored)
        #np.savetxt("../results/field.dat", self.electric)

def affichage(z,x,dim):
    fig = plt.figure(figsize=plt.figaspect(2.))
    ax = fig.add_subplot(2, 1, 2, projection='3d')
    X,Y = np.meshgrid(x[:-1,0,0], x[:-1,0,0])
